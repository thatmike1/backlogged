        ...
        <>
          {games.length === 0 ? (
            <Card className="p-8 text-center">
              <Gamepad2 className="w-16 h-16 mx-auto text-stone-300 dark:text-stone-600 mb-4" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {games.map((game) => (
                <GameDiscoverCard
                  key={game.igdbId}
                  game={game}
                  isInLibrary={libraryIgdbIds.has(game.igdbId)}
  ...
  return (
    ...
    >
      <CardContent className="p-4">
        <div className="flex gap-4">
          {/* cover */}
          <a href={`/game/${game.igdbId}`} className="shrink-0">
            {coverImage ? (
              <img
                src={coverImage}
                alt={`${game.name} cover`}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white dark:from-stone-800 pt-8">
            <div className="flex gap-2 justify-end">
              <Button
                size="sm"
                variant="success"
                onClick={() => onQuickAdd(game.igdbId, "playing")}
                disabled={isAdding}
              >
              ...
              <Button
                size="sm"
                variant="secondary"
                onClick={() => onQuickAdd(game.igdbId, "backlog")}
                disabled={isAdding}
              >
                Backlog
              </Button>
